...
impl Printer {
    ...
    pub fn print(&mut self, stmts: &Vec<&mut Statement>) {
        println!("digraph IR {{");
        for s in stmts {
            self.statement(s);
        }
    ...
    fn seq(&mut self, stmts: &Vec<&mut Statement>) {
        let idx = self.count;
        self.add_label("Seq");
        for s in stmts {
            self.add_edge(idx, self.count);
    ...
    fn _return(&mut self, r: Option<&mut Expr>) {
        match r {
    ...
    fn call(&mut self, l: Label, v: &Vec<&mut Expr>) {
        let idx = self.count;
        self.add_label("Call");
        self.add_edge(idx, self.count);
